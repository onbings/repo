...
public:
  ...
  void operator()(Param1 p1){
    ...
    InvocationNode* node = GetInvocationHead();
    while (node != 0) {
      Delegate1<Param1>* delegate = static_cast<Delegate1<Param1>*>(node->Delegate);
      pUserContext = delegate->UserContext();
      if (pUserContext)
      {
        InjectUserContext(p1, pUserContext);
      }
  ...
  void InjectUserContext(Param1 _p1, void *_pUserContext)
  {
    InjectUserContext(_p1, _pUserContext, std::is_pointer<Param1>());
  }
...
private:
  //Raw binary injection of _pUserContext as first param of the arg. The idea is to have a pointer or a ref to a STRUCTURE which MUST contains as FIRST field a pointer member
  void InjectUserContext(Param1 _p1, void *_pUserContext, std::true_type)
  {
    static_assert(std::is_pointer<Param1>::value, "This is a pointer");
    memcpy((void *)_p1, &_pUserContext, sizeof(void *));
  }
  void InjectUserContext(Param1 _p1, void *_pUserContext, std::false_type)
  {
    static_assert(!std::is_pointer<Param1>::value, "This is not a pointer");
    memcpy((void *)&_p1, &_pUserContext, sizeof(void *));
  }
  // Prevent copying objects
