...
{
public:
	static Param New(Param param,void *_pUserContext=nullptr) {	return param; }
...
{
public:
	static Param* New(Param* param, void *_pUserContext=nullptr)	{
#if USE_XALLOCATOR
    ...
    if ((newParam) && (_pUserContext))
    {
      memcpy((void *)newParam, &_pUserContext, sizeof(void *)); //Raw binary injection of _pUserContext as first param of the arg. The idea is to have a pointer or a ref to a STRUCTURE which MUST contains as FIRST field a pointer member
    }
...
public:
  ...
  void UserContext(void *_pUserContext)  {mpUserContext=_pUserContext;}
...
//  virtual void operator()(Param1 p1, typename std::enable_if<std::is_pointer<Param1>::value >::type* = nullptr) {
//  virtual void operator()(Param1 p1, typename std::enable_if<std::is_pointer<Param1>::value>) {
  virtual void operator()(Param1 p1) {
    if (m_thread == 0)
    ...
    {
      // Create a new instance of the function argument data and copy
      Param1 heapParam1 = DelegateParam<Param1>::New(p1, mpUserContext);
